...
func newSender(t *testing.T, ctx *context, store *delta.Store, client http.Client) patchSender {
	pSender, err := newPatchSenderVortex("entityKey", agentKey, ctx, store, "user-agent", ctx.AgentIdentity, NewProvideIDs(newIncrementalRegister(), state.NewRegisterSM(), nil), entity.NewKnownIDs(), client)
	require.NoError(t, err)
	return pSender
}
